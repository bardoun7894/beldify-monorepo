...
export function WishlistProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshWishlist = async () => {
    ...
    try {
      ...
      toast.error(t('wishlist.failed_to_fetch', 'Failed to load wishlist'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const addToWishlist = async (productId: number, opts?: WishlistNotifyOpts) => {
    if (!isAuthenticated) {
      ...
      try {
        ...
        toast.error(t('wishlist.failed_to_add', 'Failed to add to wishlist'));
      }
      return;
    }
  ...
  const updateWishlistNotifications = async (productId: number, opts: WishlistNotifyOpts) => {
    if (!isAuthenticated) {
      ...
      toast.error(t('wishlist.login_required', 'Please log in to manage your wishlist'));
      return;
    }
