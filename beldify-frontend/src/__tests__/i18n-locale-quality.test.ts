...
describe('i18n locale quality', () => {
  ...
  it('keeps ma/ar interpolation placeholders aligned with English', () => {
    ...
    for (const locale of ['ma', 'ar']) {
      ...
      for (const [key, englishValue] of Object.entries(en)) {
        ...
        // i18next plural forms for exact counts may drop {{count}} — "حاجة واحدة"
        // beats "1 حاجة" when the _one form only ever renders with count === 1.
        if (/_(zero|one|two)$/.test(key)) continue;
  ...
  it('keeps Moroccan Arabic copy polished and free of obvious machine wording', () => {
    ...
    // NOTE: deliberate Darija vocabulary is allowed — لقفة (cart), حانوت (shop),
    // كولشي, بروفيل, ستوك, تيليفون, كوليكسيون, حوايج, ماشي ضروري are the
    // established register of the 2026-06 copy waves and are NOT machine wording.
    for (const phrase of [
      'الكاريتو',
      'الكارو',
      'كوموند',
