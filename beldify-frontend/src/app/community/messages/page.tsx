...
export default function MessagesPage() {
  ...
  // Pin refreshUnreadCount in a ref so the fetch effect doesn't tear down when
  // the context recomputes its identity (it depends on `user`, which can churn).
  const refreshUnreadCountRef = useRef(refreshUnreadCount);
  useEffect(() => {
    refreshUnreadCountRef.current = refreshUnreadCount;
  }, [refreshUnreadCount]);

  // Token-based race guard: each fetch claims a token; setState only commits if
  // its token is still current. Replaces the cruder ignore-flag pattern so the
  // Refresh button (which calls the same fn) shares one mechanism.
  ...
  useEffect(() => {
    ...
    return () => {
      // Invalidate any in-flight fetch when auth state changes or component unmounts.
            ...
            <div className="bg-amber-50/40 rounded-2xl ring-1 ring-amber-200 p-4">
              <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                ...
                <button
                  ...
                  disabled={isLoading}
                  className="inline-flex items-center gap-2 px-4 py-2.5 min-h-[44px] text-sm font-medium text-gray-700 ring-1 ring-amber-200 rounded-full hover:ring-amber-300 hover:bg-amber-50 transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed"
                >
                  <RefreshCw size={14} />
                  {t('common.refresh')}
                </button>
