...
export async function GET(request: NextRequest, { params }: { params: { shopId: string } }) {
  try {
    ...
    logger.error('Error fetching conversation:', error.response?.data || error.message);
...
export async function PUT(request: NextRequest, { params }: { params: { shopId: string } }) {
  try {
    ...
    logger.error('Error marking conversation as read:', error.response?.data || error.message);
    return NextResponse.json(
