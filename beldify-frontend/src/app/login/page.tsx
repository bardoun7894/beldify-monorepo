...
export default function LoginPage() {
	...
	// Validate via safeRedirect before storing to prevent open-redirect attacks:
	// an attacker can send ?redirect=https://evil.com and naive code would honour
	// it after login. We discard the unsafe value here so the post-login path
	// never sees it (even if a downstream consumer forgets to validate).
	useEffect(() => {
	...
	const handleStoredAction = async () => {
		...
		try {
			...
			const quantity = parseInt(
				sessionStorage.getItem("redirectQuantity") || "1",
			);
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (!handledStoredAction) {
				...
				// safeRedirect validates the stored value is a same-origin internal
				// path; any external or protocol-relative URL is replaced with the
				// safe fallback ('/profile').
				const redirectPath = sessionStorage.getItem("redirectAfterLogin");
				sessionStorage.removeItem("redirectAfterLogin");
				router.push(safeRedirect(redirectPath, "/profile"));
			}
		} catch (error: any) {
		...
		try {
			...
			if (authResult.success) {
				...
				if (!handledStoredAction) {
					// If no stored action was handled, check for general redirect path.
					// safeRedirect validates the stored value is a same-origin internal
					// path; see the password login flow above for the same pattern.
					const redirectPath = sessionStorage.getItem("redirectAfterLogin");
					sessionStorage.removeItem("redirectAfterLogin");
					router.push(safeRedirect(redirectPath, "/profile"));
				}
				...
				<div className="w-full max-w-md">
					...
					<form className="space-y-5" onSubmit={handleSubmit} noValidate>
						...
						>
							...
							{t(
								loading ? "auth.logging_in" : "auth.login",
