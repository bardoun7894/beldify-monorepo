			...
			if (result?.success) {
				toast.success(
					t(
						...
						"Account created — welcome to Beldify!",
					),
				);
				// Validate the ?redirect= param to prevent open-redirect attacks.
				// safeRedirect returns a same-origin internal path; any external or
				// protocol-relative URL falls back to '/profile'.
				const redirect = searchParams?.get("redirect");
				router.push(safeRedirect(redirect, "/profile"));
			} else {
				const msg =
					result?.message ||
					t(
				...
				<div className="w-full max-w-lg">
					...
					<div className="mb-8">
						...
						<p className="text-gray-600 text-sm">
							{t(
								...
								"Join thousands of customers across Morocco.",
							)}
						</p>
					</div>
					...
					<form className="space-y-5" onSubmit={handleSubmit} noValidate>
						...
						<div>
							...
							<div className="relative">
								...
								<input
									...
									aria-describedby={
										fieldErrors.full_name_en ? "full_name_en-error" : undefined
									}
									className={
										fieldErrors.full_name_en ? inputError : inputDefault
									}
									placeholder={t(
								...
								<input
									...
									className={
										fieldErrors.password
											? inputWithToggleError
											: inputWithToggleDefault
									}
									placeholder="••••••••"
								/>
								<button
							...
							{formData.password && strengthMeta && (
								<div className="mt-2 flex items-center gap-3">
									<div className="h-1 flex-1 rounded-full bg-amber-100 overflow-hidden">
										<div
											className={`h-full rounded-full transition-all duration-200 ${strengthMeta.bar}`}
										/>
									</div>
						...
						>
							...
							{t(
								loading ? "auth.registering" : "auth.register",
