...
const AUTH_SELLER = {
	...
	user: { role: "seller", is_seller: true },
};
...
describe("SellerDashboardPage", () => {
	...
	it("renders recent orders table with order numbers", async () => {
		const { default: Page } = await import("../page");
		render(<Page />);
		// Text appears in both mobile card list and desktop table
		const ord1 = await screen.findAllByText("ORD-001");
		expect(ord1.length).toBeGreaterThanOrEqual(1);
		const ord2 = screen.getAllByText("ORD-002");
		expect(ord2.length).toBeGreaterThanOrEqual(1);
	});
	...
	it("renders customer names in recent orders", async () => {
		...
		render(<Page />);
		// Text appears in both mobile card list and desktop table
		const customers = await screen.findAllByText("Ahmed B.");
	...
	it("renders onboarding status indicator", async () => {
		...
		const statusEl = await screen.findByText(
			/complete your store setup|90% done/i,
		);
	...
	it("renders orders even when earnings call fails (graceful degradation)", async () => {
		...
		const { default: Page } = await import("../page");
		render(<Page />);
		// Orders still show despite earnings failure (in both mobile cards and desktop table)
		const ords = await screen.findAllByText("ORD-001");
		expect(ords.length).toBeGreaterThanOrEqual(1);
	});
