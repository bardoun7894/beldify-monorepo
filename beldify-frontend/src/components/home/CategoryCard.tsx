...
export default function CategoryCard({
	...
	const displayName = isArabicScript
		? name_ar || name_en
		: name_en || name_ar || "";
	...
	return (
		<Link
			href={href}
			aria-label={t("home.categories.shop_all", "Shop all {{name}}", {
				name: displayName,
			})}
			className={[
		...
		>
					...
					{typeof itemCount === "number" && itemCount > 0 && (
						...
						>
							<Package
								...
								aria-hidden="true"
							/>
							{t("home.categories.products_count", "{{count}} products", {
								count: itemCount,
							})}
						</p>
					)}
			...
			<div className="flex flex-1 flex-col gap-3 p-4">
									...
									>
										<span className="relative grid h-5 w-5 shrink-0 place-items-center overflow-hidden rounded-full bg-indigo-100">
											{subHasImage ? (
												...
												<span
													className="text-[10px] font-bold text-indigo-700/70"
													aria-hidden="true"
												>
						...
						{hiddenCount > 0 && (
							<li>
								<span className="inline-flex items-center rounded-full bg-indigo-50 px-2.5 py-1 text-[11px] font-medium text-indigo-700 ring-1 ring-indigo-100">
									{t("home.categories.sub_chips_more", "+{{n}} more", {
										n: hiddenCount,
									})}
								</span>
							</li>
				...
				>
					{t("home.categories.shop_all", "Shop all {{name}}", {
						name: displayName,
					})}
					<ArrowRight
