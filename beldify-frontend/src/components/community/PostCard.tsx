...
function formatBudget(post: CommunityPost, numLocale: string): string {
  if (post.budget && typeof post.budget === 'object') {
    ...
    if (!isNaN(min) && !isNaN(max)) {
      ...
      return `${min.toLocaleString(numLocale)} – ${max.toLocaleString(numLocale)} ${currency}`;
    }
  }
...
export default function PostCard({ post, isUserPost = false }: PostCardProps) {
  ...
  const budgetDisplay = formatBudget(post, numLocale);
