...
export default function NotifyMeButton({
	...
	const { isInWishlist, addToWishlist, updateWishlistNotifications } =
		useWishlist();
			...
			if (!isSupported) {
				toast.success(
					t(
						...
						"Saved. Your browser does not support notifications",
					),
				);
			} else if (!pushGranted) {
				toast.success(
					t(
						...
						"Saved. Enable notifications to receive alerts",
					),
				);
			} else {
