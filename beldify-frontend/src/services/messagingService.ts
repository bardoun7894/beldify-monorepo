...
export const getRecentMessages = async (): Promise<Message[]> => {
	try {
		...
		const response = await axios.get(
			`${MESSAGING_ENDPOINTS.frontend}/conversations`,
			{
...
export const getConversation = async (
	...
	try {
		...
		const response = await axios.get(
			`${MESSAGING_ENDPOINTS.frontend}/conversations/${userId}`,
			{
				params,
				headers: getAuthHeaders(),
				withCredentials: true,
...
export const getSellerConversations =
	async (): Promise<SellerConversationsResult> => {
		const empty: SellerConversationsResult = {
			conversations: [],
			total_unread: 0,
		};
		try {
...
export const getSellerUnreadCount = async (): Promise<number> => {
	try {
		...
		const response = await axios.get(
			`${API_BASE_URL}/api/v1/backend/messages/unread-count`,
			{
				headers: getAuthHeaders(),
				withCredentials: true,
			},
...
export const getUnreadCount = async (
	...
	try {
		...
		const response = await axios.get(
			`${MESSAGING_ENDPOINTS.buyer}/unread-count`,
			{
				params,
				headers: getAuthHeaders(),
				withCredentials: true,
...
export const markMessagesAsRead = async (
	...
	try {
		...
		if (messageId) {
			logger.log(
				"Message will be marked as read automatically when conversation is viewed",
			);
			return { count: 1 };
		}
		...
		if (shopId) {
			logger.log(
				"Messages will be marked as read automatically when conversation is viewed",
			);
			return { count: 1 };
		}
...
export const checkNewMessages = async (
	...
	try {
		...
		const response = await axios.get(
			`${MESSAGING_ENDPOINTS.buyer}/shops/${shopId}`,
			{
				params,
				headers: getAuthHeaders(),
				withCredentials: true,
