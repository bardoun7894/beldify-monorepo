...
export async function submitStoreRequest(
  ...
  try {
    ...
    if (error.response?.status === 422) {
      ...
      const msg = Array.isArray(firstError) ? firstError[0] : error.response.data?.message;
      return { success: false, message: msg || 'Validation failed' };
    }

    return {
      success: false,
      message: error.response?.data?.message || error.message || 'Failed to submit store request',
    };
  }
}
